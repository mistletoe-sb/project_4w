...
public class MemberDAO implements DAOInterface<MemberVO>{
	...
	public void insertData(MemberVO vo) {
		...
		try {
			PreparedStatement pstmt = conn.prepareStatement(sql);		// 실행할 insert문 세팅
	...
	@Override
	// update 실행 메소드 구현
	public void updateData(MemberVO vo) {
		String sql = "";
		boolean hasPW = false;
		...
		try {
			PreparedStatement pstmt = conn.prepareStatement(sql);		// 실행할 update문 세팅
	...
	@Override
	// delete 실행 메소드 구현
	public void deleteData(int mem_num) {
		// 실행할 delete문 작성
		String sql = "delete from pj_4w.MEMBERS where mem_num = ?";
		try {
			PreparedStatement pstmt = conn.prepareStatement(sql);		// 실행할 delete문 세팅
	...
	public ArrayList<MemberVO> selectAllData() {
		...
		try {
			PreparedStatement pstmt = conn.prepareStatement(sql);		// 실행할 select문 세팅
			ResultSet rs = pstmt.executeQuery();				// select문 실행 후 결과를 ResultSet 객체에 저장
